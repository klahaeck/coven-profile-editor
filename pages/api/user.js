...
export default async function User(req, res) {
  ...
  if (req.method == 'POST') {
    try {
      ...
      const token = await fetch(`${process.env.AUTH0_ISSUER_BASE_URL}/oauth/token`, {
        method: 'POST',
        headers: {
      ...
      const updatedUser = await fetch(`${process.env.AUTH0_ISSUER_BASE_URL}/api/v2/users/${user.sub}`, {
        method: 'PATCH',
        headers: {
